package org.openscience.webcase.createpylsdinputfile.utils;

import org.openscience.webcase.createpylsdinputfile.model.nmrdisplayer.Correlation;
...
public class HybridizationDetection {
    ...
    public static Map<Integer, List<Integer>> getDetectedHybridizations(final WebClient.Builder webClientBuilder, final Data data, final double thrs) {
        final Map<Integer, List<Integer>> detectedHybridizations = new HashMap<>();

        final WebClient webClient = webClientBuilder.
                baseUrl("http://localhost:8081/webcase-db-service-hybridization/nmrshiftdb")
        ...
        for (int i = 0; i < data.getCorrelations().getValues().size(); i++) {
            ...
            if (multiplicity != null) {
                ...
                uriComponentsBuilder.path("/detectHybridizations")
                        ...
                        .queryParam("minShift", (int) correlation.getSignal().getDelta() - 2)
                        .queryParam("maxShift", (int) correlation.getSignal().getDelta() + 2)
